...
class Arduino(ControlSurface):
    ...
    def __init__(self, *a, **k):
        _LOG(f"A | {a}")
        _LOG(f"K | {k}")
        super(Arduino, self).__init__(ControlSurfaceSpecification(elements_type=ElementsBase), *a, **k)
    ...
    def setup(self):
        ...
        mixer.master_strip.volume_control.set_control_element(CustomEncoderElement(identifier=20, channel=1, is_feedback_enabled=True))
        mixer.set_enabled(True)